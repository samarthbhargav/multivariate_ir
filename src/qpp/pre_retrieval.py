...
if __name__ == "__main__":
    ...
    if args.mode == "precomputation":
        ...
        for qid, qtext in tqdm(query.items(), total=len(query)):
            ...
            # PMI
            predicted_performance[qid]["PMI-avg"], predicted_performance[qid]["PMI-max"], predicted_performance[qid][
                "PMI-sum"] = avg_max_sum_PMI(qtokens, index_reader, qtoken2did)

            # ql
            predicted_performance[qid]["ql"] = len(qtokens)

            # VAR (var)
            predicted_performance[qid]["VAR-var-avg"], predicted_performance[qid]["VAR-var-max"], \
            predicted_performance[qid]["VAR-var-sum"] = avg_max_sum_VAR(qtokens, qtoken2var)

            # VAR (std)
            predicted_performance[qid]["VAR-std-avg"], predicted_performance[qid]["VAR-std-max"], \
            predicted_performance[qid]["VAR-std-sum"] = avg_max_sum_VAR(qtokens, qtoken2std)

            # IDF
            predicted_performance[qid]["IDF-avg"], predicted_performance[qid]["IDF-max"], predicted_performance[qid][
                "IDF-sum"], predicted_performance[qid]["IDF-std"] = avg_max_sum_std_IDF(qtokens, index_reader)

            # SCQ
            predicted_performance[qid]["SCQ-avg"], predicted_performance[qid]["SCQ-max"], predicted_performance[qid][
                "SCQ-sum"] = avg_max_sum_SCQ(qtokens, index_reader)
        ...
        for qpp_name in args.qpp_names:
            ...
            for qid, qtext in query.items():
                ...
                predicted_performance[qid][f"{qpp_name}-ppl-{args.LM}-{args.alpha}_mm"] = args.alpha * min_max_ppl + (
                        (1.00 - args.alpha) * min_max_qpp)
                predicted_performance[qid][f"{qpp_name}-ppl-{args.LM}-{args.alpha}_z"] = args.alpha * z_ppl + (
                        (1.00 - args.alpha) * z_qpp)
    ...
    if args.mode != "precomputation":
        ...
        for name in name_list:
            output_path_ = f"{args.output_path}/{name}"
            print(f"{name}")
