...
def main():
    ...
    train_dataset = datasets.load_dataset(
        data_args.dataset_name, data_args.dataset_language, cache_dir=model_args.cache_dir, data_files=data_files
    )[data_args.dataset_split]

    def tokenize_train(example):
        tokenize = partial(
            tokenizer, return_attention_mask=False, return_token_type_ids=False, padding=False, truncation=True
        )
        query = example["query"]
    ...
    if training_args.grad_cache:
        ...
        p_train_step = jax.pmap(
            partial(grad_cache_train_step, q_n_subbatch=q_n_subbatch, p_n_subbatch=p_n_subbatch), "device"
        )
    else:
